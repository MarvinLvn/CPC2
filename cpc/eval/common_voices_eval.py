...
class SingleSequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return outSeq, torch.tensor([sizeSeq], dtype=torch.long), \
            outPhone.long(),  torch.tensor([sizePhone], dtype=torch.long)
...
class CTCphone_criterion(torch.nn.Module):
    ...
    def __init__(self, dimEncoder, nPhones, LSTM=False, sizeKernel=8,
        ...
        from cpc.criterion.custom_layers import EqualizedConv1d
        ...
        #self.PhoneCriterionClassifier = EqualizedConv1d(dimEncoder, nPhones + 1, sizeKernel, stride=sizeKernel // 2, lrMul=10)

    def getPrediction(self, cFeature, featureSize):
        B, S, H = cFeature.size()
        if self.seqNorm:
            x = torch.randn(cFeature.size(), device = cFeature.device)
            for b in range(B):
                size = featureSize[b]
...
if __name__ == "__main__":
    ...
    parser_train.add_argument('--loss_reduction', type=str, default='mean',
                              choices=['mean', 'sum'])
    parser_train.add_argument('--roffset', type=int, default=0)
    parser_train.add_argument('-a', '--augments', type=json.loads, nargs='*',
                             help="I KNOW I AM DIRTY DON'T JUDGE ME.",
                             default=None)
    parser_train.add_argument('--t_ms',type=int, default=100)
    ...
    if args.command == 'train':
        ...
        if not args.freeze:
            ...
            optimizer = torch.optim.AdamW(
                                         ...
                                         {'params': feature_maker.parameters(), 'lr': args.lr / 10}],
                                         lr=args.lr,
                                         betas=(args.beta1, args.beta2),
                                         eps=args.epsilon)
        else:
            optimizer = torch.optim.AdamW(list(phone_criterion.parameters()), lr=args.lr,
                                          betas=(args.beta1, args.beta2),
                                          eps=args.epsilon)
