...
import torch
from cpc.criterion.clustering import kMeanGPU, kMeanCluster
...
def parseArgs(argv):
    ...
    parser.add_argument('--level_gru', type=int, default=0,
                        help='Specify the LSTM hidden level to take the representation (default: 0, '
                             'which means last level of representation). 1 means, we take the last-1 layer, and so on')
...
if __name__ == "__main__":
    ...
    model = loadModel([args.pathCheckpoint], intermediate_idx=args.level_gru)[0]
